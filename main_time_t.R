...
### 6 methods: CLIME, Trans-CLIME, Trans-CLIME(oracle),
###            RS-CLIME, Trans-TE-CLIME, Trans-TE-CLIME(oracle)
### 2 function: CLIME and Trans-CLIME, Trans-TE-CLIME and RS-CLIME
### input: dat.all, const
### output: FPR, TPR, FPR.ori, TRP.ori, FPR.o, TPR.o
###         Omega, Omega.ori, Omega.o
...
Trans.CLIME.all <- function(dat.all,const){
  library(mvtnorm)
  library(fastclime)
  library(Matrix)
  ...
  Theta.re01<-Myfastclime.s(X=X.all[1:n0,], Bmat=diag(1,p), lambda=const*2*sqrt(log(p)/n0))
  Theta.init1<-Theta.re01$Theta.hat
  Omega.otl1 <- Trans.CLIME(X=X.all[1:n0,], X.A=X.all[(n.vec[1]+1):(n.vec[1]+nA),], const=const,
                            X.til= X.all[(n0+1):n.vec[1],], Theta.cl=Theta.init1)
  ind2<-(n.vec[1]-n0+1): n.vec[1]
  Theta.re02<-Myfastclime.s(X=X.all[ind2,], Bmat=diag(1,p), lambda=const*2*sqrt(log(p)/n0))
  Theta.init2<-Theta.re02$Theta.hat
  Omega.otl2 <- Trans.CLIME(X=X.all[ind2,], X.A=X.all[(n.vec[1]+1):(n.vec[1]+nA),], const=const,
                            X.til= X.all[1:(n.vec[1]-n0),], Theta.cl=Theta.init2)
  Omega.otl<-(Omega.otl1+Omega.otl2)/2
  Omega.otl.s <- (Omega.otl+t(Omega.otl))/2
  ...
  E.est <- (Omega.tl.s != 0)
  E.true <- (Theta0 != 0)
  FPR <- (sum(E.true==0 & E.est==1)/2)/(sum(!E.true)/2)
  TPR <- ((sum(E.true==1 & E.est==1)-p)/2)/((sum(E.true)-p)/2)
  
  Theta.hat0.s <- (Theta.hat0+t(Theta.hat0))/2
  E.ori <- (Theta.hat0.s != 0)
  FPR.ori <- (sum(E.true==0 & E.ori==1)/2)/(sum(!E.true)/2)
  TPR.ori <- ((sum(E.true==1 & E.ori==1)-p)/2)/((sum(E.true)-p)/2)
  
  Theta.otl.s <- (Omega.otl+t(Omega.otl))/2
  E.o <- (Theta.otl.s != 0)
  FPR.o <- (sum(E.true==0 & E.o==1)/2)/(sum(!E.true)/2)
  TPR.o <- ((sum(E.true==1 & E.o==1)-p)/2)/((sum(E.true)-p)/2)
  
  return(list(FPR=FPR, TPR=TPR, FPR.ori=FPR.ori, TPR.ori=TPR.ori,
              FPR.o=FPR.o, TPR.o=TPR.o,
              Omega.ori=Theta.hat0.s, Omega=Omega.tl.s, Omega.o=Theta.otl.s))
}

te.trans.clime <- function(dat.all,const){
  library(mvtnorm)
  library(fastclime)
  library(Matrix)
  library(pcaPP)
...
te.trans.clime.new <- function(dat.all,const){
  library(mvtnorm)
  library(fastclime)
  library(Matrix)
  library(pcaPP)
...
### data generation
### input: K auxilary studies, A.size imformative studies,
###        n.vec: sample size
###        h: a tuning parameter determine the distance between data and auxilary studies
### distr = norm, t, ell, te, npnorm
### norm: no additional parameter
### t: add.para[1] is parameter of t distribution
### ell: add.func1 is the rad distribution, univariate
### te: add.func1, add.func2 is the transform funciton, p-dimension
### npnorm: add.func2, p-dimension
###
### 
...
parafunc <- function(s,h){
  ...
  start_t1 <- Sys.time()
  t1 <- tryCatch(te.trans.clime.new(dat.all, const), error = function(err) return(NULL))
  end_t1 <- Sys.time()
  elapsed_t1 <- end_t1 - start_t1
  
  if (model != "ell") {
    start_t2 <- Sys.time()
    t2 <- tryCatch(Trans.CLIME.all(dat.all, const), error = function(err) return(NULL))
    end_t2 <- Sys.time()
  ...
  cat("t1 time:", elapsed_t1, "\n")
  cat("t2 time:", elapsed_t2, "\n")
  ...
  if(is.null(t2)){
    # return(c(t1$FPR,t1$TPR,t1$FPR.ori,t1$TPR.ori,t1$FPR.o,t1$TPR.o,
    #          rep(0,6),
    #          norm(t1$Omega-dat.all$Theta0, 'F')^2/p,norm(t1$Omega-dat.all$Theta0, '2')^2,
    #          norm(t1$Omega.ori-dat.all$Theta0, 'F')^2/p, norm(t1$Omega.ori-dat.all$Theta0, '2')^2,
    #          norm(t1$Omega.o-dat.all$Theta0, 'F')^2/p,norm(t1$Omega.o-dat.all$Theta0, '2')^2,
    #          rep(0,6)))
    # 
    # AUC_norm(omega,estimator)
    # 
    # AUC_norm(dat.all$Theta0,)
    # 
    # F_norm(omega,estimator)
    # 
    # F_norm(dat.all$Theta0,)
    
    # return(c(AUC_norm(dat.all$Theta0,t1$Omega),AUC_norm(dat.all$Theta0,t1$Omega.ori),
    #          AUC_norm(dat.all$Theta0,t1$Omega.o),rep(0,3),
    #          F_norm(dat.all$Theta0,t1$Omega),F_norm(dat.all$Theta0,t1$Omega.ori),
    #          F_norm(dat.all$Theta0,t1$Omega.o),rep(0,3)))
    
    
    return(c(elapsed_t1,rep(0,5),
             0,rep(0,5)))
        ...
        for(p in p.list){
          ...
          for(const in const.list){
            ...
            clusterExport(cl, varlist=c("add.func2","h.1","add.func3","add.func1",
                                        "te.trans.clime","Trans.CLIME.all","Theta0",
            ...
            print(c(model, A.size, Theta0.type, p, const))
          }
          total.res[[paste0(model,".h",h,".",A.size,".",Theta0.type,".",p)]] <- all.res
        }
      }
    }
  }
...
write.csv(best.res, paste(rep.times,models, "best.res.time.csv", sep = "_"), row.names = names(total.res))
